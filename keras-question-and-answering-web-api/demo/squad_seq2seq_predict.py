...
def f1_score(prediction, ground_truth,larb_score):
    if prediction == ground_truth:
        return 1
    prediction_tokens = _normalize_answer(prediction).split()
    ground_truth_tokens = _normalize_answer(ground_truth).split()
    common = Counter(prediction_tokens) & Counter(ground_truth_tokens)
#    larb_score+=label_ranking_average_precision_score(ground_truth_tokens,prediction_tokens)
...
def main():
    ...
    for i in range(20):
        ...
        score+=f1_score(predicted_answer,actual_answer,larb_score)
    score/=20
    print("f1_score "+str(score))
#    print("larb_score "+str(larb_score/20))
